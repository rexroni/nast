...
int fstrlit(FILE *f, char *s){
#   define CHR(x) buf[len++] = x
    char buf[128];
    size_t len = 0, maxlen = sizeof(buf);
    char c;
    size_t i;
    for(i = 0, c = s[i]; len < maxlen && c != '\0'; c = s[++i]){
        if     (c == '\0'){ CHR('\\'); CHR('0'); }
...
int run_logger(char *logfile){
    ...
    while(1){
        ...
        if(read_string(norm, sizeof(norm))){
            retval = 1;
            goto cu;
        }
        ...
        fflush(stdout);
        if(read_string(curkey, sizeof(curkey))){
            retval = 1;
            goto cu;
        }
        ...
        fprintf(f, "%s,", name);
        fstrlit(f, norm); fprintf(f, ",");
        fstrlit(f, cur); fprintf(f, ",");
        fstrlit(f, key); fprintf(f, ",");
        fstrlit(f, curkey); fprintf(f, "\n");
    }
